-- LedgerWorks V010: scope posting bypass to entry id and restore prior settings
...
BEGIN
  IF current_setting('app.allow_posting', true) = 'on' THEN
    posting_entry := NULLIF(current_setting('app.posting_entry_id', true), '')::uuid;
    IF posting_entry IS NOT NULL AND posting_entry = NEW.id THEN
      RETURN NEW;
    END IF;
  END IF;
